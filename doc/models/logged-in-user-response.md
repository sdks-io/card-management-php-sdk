...
| `decimalSeparator` | `?string` | Optional | Preferred Decimal separator configured for the logged in user<br>Note: - Colco default value returned when the user is not provided or does not exist. | getDecimalSeparator(): ?string | setDecimalSeparator(?string decimalSeparator): void |
